...
namespace HRWorkForceSystemBackend.Controllers
{
    ...
    public class UserManagementController : ControllerBase
    {
      ...
      /*  private async Task<string> GenerateUniqueWorkforceIdAsync()
        ...
        public async Task<IActionResult> AddWorkforceDetails([FromBody] AddWorkforceDetailsDto dto)
        {
           ...
           // if (!string.IsNullOrEmpty(workforceUser.WorkforceId))
              //  return BadRequest("Workforce details already assigned.");

            // Generate a unique Workforce ID
            //string generatedId = await GenerateUniqueWorkforceIdAsync();

            // Assign details
           // workforceUser.WorkforceId = generatedId;
            workforceUser.Department = dto.Department;
            workforceUser.CurrentRole = dto.CurrentRole;

            _context.WorkforceUsers.Update(workforceUser);
            await _context.SaveChangesAsync();

            return Ok(new
            {
                Message = "Workforce details added successfully.",
                //WorkforceId = generatedId
            });
        }
